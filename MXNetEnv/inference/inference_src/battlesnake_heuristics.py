...
class MyBattlesnakeHeuristics:
    ...
    def size_bigger_than_others(self,json):
        max_length =0
        for i in (json['board']['snakes']):
            temp_length = len(i['body'])
            if (temp_length>max_length):
                max_length = temp_length
                
        if ( len(json["you"]["body"]) <= max_length ):
            return False
    ...
    def run(self, state, snake_id, turn_count, health, json, action):
        ...
        if health[snake_id] < health_thr and np.random.uniform()>0.5 or (not self.size_bigger_than_others(json)):
